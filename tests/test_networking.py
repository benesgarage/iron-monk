...
def test_url_constraint():
    ...
    with pytest.raises(ValueError): URL.validate("url", "not-a-url")
    with pytest.raises(ValueError): URL.validate("url", "http://")  # No network location


def test_ipaddress_constraint():
    ...
    with pytest.raises(ValueError): IPAddress.validate("ip", "256.256.256.256")


def test_uuid_constraint():
    UUID.validate("id", "123e4567-e89b-12d3-a456-426614174000")
    UUID.validate("id", uuid.uuid4())  # Native object
    UUID.validate("id", None)
    
    with pytest.raises(ValueError): UUID.validate("id", "not-a-uuid")