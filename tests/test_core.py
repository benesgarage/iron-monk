...
def test_validate_non_monk_object():
    with pytest.raises(TypeError):
        validate(123)


def test_structural_subtyping_duck_typing():
...
def test_not_inverter_constraint():
...
def test_is_utc_constraint():
    ...
    with pytest.raises(ValueError): IsUTC.validate("dt", dt_naive)
    with pytest.raises(ValueError): IsUTC.validate("dt", dt_other)
...
def test_slotted_dataclass_lifecycle():
...
def test_deferred_validation_kwarg():
...
def test_global_config_deferred_validation():
...
def test_env_var_global_config(monkeypatch):
...
def test_constraint_auto_instantiation():
...
def test_constraint_auto_instantiation_missing_args():
    @constraint
